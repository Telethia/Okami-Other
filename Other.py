...
def Main():
    ...
    def TRE():
        ...
        while i < EntryCount:
            ...
            if Vers == "f":
                PaddingHD = int.from_bytes(FileOpen.read(8), 'little')
            else:
                pass
            Text = "Entry: " + str(i) + "\n"
            OutputFile.write(Text)
    ...
    def ANS():
        ...
        while i < EntryCount:
            print(str("0x" + format(FileOpen.tell(), 'x')))
            AnimalID = int.from_bytes(FileOpen.read(1), 'little')
            AnimalCat = int.from_bytes(FileOpen.read(1), 'little')
            Unk2 = int.from_bytes(FileOpen.read(2), 'little')
            SizeX = int.from_bytes(FileOpen.read(1), 'little', signed=True)
            SizeY = int.from_bytes(FileOpen.read(1), 'little', signed=True)
            SizeZ = int.from_bytes(FileOpen.read(1), 'little', signed=True)
            RotX = int.from_bytes(FileOpen.read(1), 'little', signed=True)
            ...
            CoordsY = int.from_bytes(FileOpen.read(2), 'little', signed=True)
            CoordsZ = int.from_bytes(FileOpen.read(2), 'little', signed=True)
            Texture = int.from_bytes(FileOpen.read(1), 'little')
            RestoreID = int.from_bytes(FileOpen.read(1), 'little')
            GroupID = int.from_bytes(FileOpen.read(1), 'little')
            Unk22 = int.from_bytes(FileOpen.read(2), 'little')
            AnimalID2 = int.from_bytes(FileOpen.read(1), 'little')
            Unk1 = int.from_bytes(FileOpen.read(1), 'little')
            AnimalGrpNr = int.from_bytes(FileOpen.read(1), 'little')
            #PraiseAmt = int.from_bytes(FileOpen.read(1), 'little')
            Padding = int.from_bytes(FileOpen.read(11), 'little')
            if Vers == "f":
                PaddingHD = int.from_bytes(FileOpen.read(8), 'little')
            else:
                pass
            ...
            OutputFile.write(Text)
            if AnimalGrpNr == 1:
                Text = str(AnimalGrpNr) + " " + AnType[AnimalID] + "\n"
            else:
                if AnType[AnimalID] == "Fox":
                    Text = str(AnimalGrpNr) + " " + AnType[AnimalID] + "es \n"
                elif AnType[AnimalID] == "Mouse":
                    Text = str(AnimalGrpNr) + " " + "Mice \n"
                else:
                    Text = str(AnimalGrpNr) + " " + AnType[AnimalID] + "s \n"
            OutputFile.write(Text)
            Text = "Size is " + str(SizeX) + ", " + str(SizeY) + ", " + str(SizeZ) + "\n"
            OutputFile.write(Text)
            Text = "Rotation is " + str(RotX) + ", " + str(RotY) + ", " + str(RotZ) + "\n"
            ...
            Text = "Coords are " + str(CoordsX) + ", " + str(CoordsY) + ", " + str(CoordsZ) + "\n"
            OutputFile.write(Text)
            #Text = "Praise from feeding is " + str(PraiseAmt) + "\n"
            #OutputFile.write(Text)
            Text = "~~~~~~~~~~~~~~~~~~~~\n\n"
            OutputFile.write(Text)
            i += 1
